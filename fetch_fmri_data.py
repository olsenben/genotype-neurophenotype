import subprocess
import os

"""
Run to fetch data via datalab. change basepath to your directory.
Make sure you have datalab installed.

clone the dataset (do not download large files yet)
datalad install https://github.com/OpenNeuroDatasets/ds004796.git 

Navigate to the dataset
cd file_path/ds004796

Use to pull EEG files (eeg + vhdr + vmrk + tsv)

Do not rename or move files from this directory!

Note: for me, the get always fails at subject 19. no idea why. it will skip but you can manually download it and replace the eeg file
"""

base_path = "E:/neuro_data/ds004796"

for i in range(1, 81):
    subject = f"sub-{i:02d}"
    nii_file = os.path.join(base_path, subject, "func", f"{subject}_task-rest_dir-PA_bold.nii.gz")
    tsv_file = os.path.join(base_path, subject, "func", f"{subject}_task-rest_dir-PA_bold.json")
    subprocess.run(["datalad", "get", nii_file, tsv_file])